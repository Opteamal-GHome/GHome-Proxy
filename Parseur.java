...
/**
 * 
 * Thread r�cup�rant les t�ches re�ues, les parsant, puis les envoyant au serveur GHome
 *
 */
public class Parseur implements Runnable {
	...
	public List<ProxyTrame> parseTrame(char[] trame) {
		...
		else if(!teachIn)// Trame de donn�es
		{
			// remettre le watchdog � 0 (car on a bien re�u une trame de ce capteur)
...
//		}
	...
	public static List<ProxyTrame> parseTypeLTO (char[] trame, DevicePhysique devPhysique, long timestamp)
	{
		...
		System.out.println("&&&&&&&&&&&&& Parseur : dans parseTypeLTO luminosité = "+luminosite);
		...
		while(continuer){
			...
			if((listeProxyTrames != null) && (listeProxyTrames.size() != 0)){
				...
				for(int i=0; i<listeProxyTrames.size(); i++) {
					// Mettre la trame dans la liste des trames � envoyer
